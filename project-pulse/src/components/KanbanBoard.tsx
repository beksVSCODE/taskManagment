...
const COLUMNS: { id: TaskStatus; label: string; color: string; bg: string; dot: string }[] = [
  ...
  { id: 'REVIEW',    label: 'На проверке', color: 'text-violet-600', bg: 'bg-violet-50/80',  dot: 'bg-violet-500' },
...
export function KanbanBoard({ tasks, users, project, onTaskClick, onCreateClick, onVoiceCreateClick }: Props) {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    // Блокируем повторное перетаскивание, пока предыдущая мутация ещё в процессе
    if (updateTask.isPending) return;
    const taskId = result.draggableId;
    const task = tasks.find(t => t.id === taskId);
    if (!task || !canDragTask(task, project)) return;
    const newStatus = result.destination.droppableId as TaskStatus;
    if (result.source.droppableId === newStatus) return;
    // Фронтенд-валидация матрицы (бекенд повторит проверку с ошибкой 400)
    if (!isTransitionAllowed(task.status, newStatus)) return;
      ...
      <DragDropContext onDragEnd={onDragEnd} onDragStart={onDragStart}>
        <div className="flex gap-3 sm:gap-4 min-h-[60vh] md:h-[calc(100vh-17rem)] overflow-x-auto pb-2">
          {COLUMNS.map(col => {
            ...
            // Колонка недоступна для дропа, если тащимая задача не может туда перейти
            const colDisabled =
              draggingTask != null &&
              draggingTask.status !== col.id &&
              !isTransitionAllowed(draggingTask.status, col.id);
            return (
              <Droppable droppableId={col.id} key={col.id} isDropDisabled={colDisabled}>
                {(provided, snapshot) => (
                  <div
                  ...
                  >
                    ...
                    >
                      {columnTasks.map((task, index) => {
                        const dragDisabled = !canDragTask(task, project) || updateTask.isPending;
                        return (
                          <Draggable key={task.id} draggableId={task.id} index={index} isDragDisabled={dragDisabled}>
                            {(provided, snapshot) => (
                              <div
                                ...
                                className={`transition-transform ${snapshot.isDragging ? 'opacity-90 rotate-1 scale-105' : ''} ${dragDisabled ? 'opacity-80 cursor-not-allowed' : 'cursor-grab active:cursor-grabbing'}`}
                                title={
                                  updateTask.isPending
                                    ? 'Ожидание ответа сервера...'
                                    : dragDisabled
                                    ? 'Недостаточно прав для смены статуса этой задачи'
                                    : 'Перетащите задачу в другую колонку'
                                }
                              >
